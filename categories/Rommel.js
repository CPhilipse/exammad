...
type Props = {};
export default class Rommel extends Component<Props> {
    constructor(props) {
    ...
    handleEmail = () => {
        Mailer.mail({
            ...
            // body: 'Omschrijving: ' & global.omschrijving & ' | ' & global.date & 'Locatie: ' & global.location & 'Categorie: ' & global.category & '.',
    ...
    renderRommel () {
        store.get('incidents')
            .then((res) =>
                    global.ress = res,
                // How do I retrieve this res out of the scope? Global is so not clean.
            );
        console.log(global.ress);
        console.log('test');
        global.ress.forEach((element, index, array) => {
            // If an object has the value of rommel show all the values of that object which has a value with rommel.
            if (element.category === 'rommel') {
                console.log('I\'M IN');
    ...
    render() {
        // const climate = global.ress.find(incident => incident.category === 'rommel');
        // if (climate.category === 'rommel') {
        //     this.setState({
        //         rommel: true,
        //     })
        // }
        return (
            <View>
                {this.state.rommel === false ? this.renderRommel() : alert('Er zijn geen rommel incidenten.')}
            </View>
        );
    }
}
