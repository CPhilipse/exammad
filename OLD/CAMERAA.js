...
// SAME with the TextInput, it re-renders with every character change. While it should be saved in the state when pressed on submit button.

// PROBLEM:: Hans checked, it is weird this page executes handleSubmit() while it is only being called on the TouchableOpacity onPress. Since there shouldn't be anything wrong with this code.
// CONSEQUENCE:: This way I can't navigate to Home when onPress of 'Verzenden', because if I add that navigate line commented out in handleSubmit, it'll give an error, because it executes that function before you press the button.
